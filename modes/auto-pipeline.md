...
When the user pastes a JD (Job Description text or URL) without an explicit sub-command, execute the ENTIRE pipeline in sequence:
...
**If the input is JD text** (not URL): use it directly, without needing to fetch.

## Step 1 — Evaluation A-G
Execute exactly the same as the `offer` mode (read `modes/offer.md` for all blocks A-F + Block G Posting Legitimacy).

## Step 2 — Save .md Report
Save the complete evaluation in `reports/{###}-{company-slug}-{YYYY-MM-DD}.md` (see format in `modes/offer.md`).
