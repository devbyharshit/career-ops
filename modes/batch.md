# Mode: batch — Bulk Processing of Offers
...
Claude Conductor (claude --chrome --dangerously-skip-permissions)
  ...
  │  Reads direct DOM — the user sees everything in real-time
  │
...
Each worker is a child `claude -p` with a clean context of 200K tokens. The conductor only orchestrates.
...
- If it dies → re-execute → reads `batch-state.tsv` → skips completed
...
| Error | Recovery |
|-------|----------|
| Unreachable URL | Worker fails → conductor marks `failed`, next |
