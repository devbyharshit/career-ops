...
Read `cv.md`. Create a table with each JD requirement mapped to exact lines in the resume.
...
3. **"If downleveled" plan**: accept if comp is fair, negotiate a 6-month review, clear promotion criteria
...
Table with data and cited sources. If there is no data, state it instead of making it up.
...
Top 5 changes to the resume + Top 5 changes to LinkedIn to maximize match.
