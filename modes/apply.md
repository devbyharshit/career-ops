...
Interactive mode for when the candidate is filling out an application form in Chrome. It reads what is on screen, loads the previous context of the offer, and generates personalized answers for each form question.

## Requirements

- **Better with visible Playwright**: In visible mode, the candidate sees the browser and Claude can interact with the page.
...
5. COMPARE   → Does the role on screen match the evaluated one? If it changed → notify
...
5. If there is NO match → notify and offer to run quick auto-pipeline
...
- [Personalization suggestions that the candidate should review]
```
