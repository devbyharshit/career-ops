...
**IMPORTANT**: This prompt is self-contained. You have EVERYTHING necessary here. You don't depend on any other skill or system.
...
**RULE: For article metrics, article-digest.md takes precedence over cv.md.** cv.md might have older numbers — this is normal.
...
**Transversal advantage**: Frame profile as **"Technical builder"** adapting their framing to the role:
...
Save complete evaluation in:
```
reports/{{REPORT_NUM}}-{company-slug}-{{DATE}}.md
```

Where `{company-slug}` is the company name in lowercase, without spaces, with dashes.
...
**IMPORTANT:** The TSV order has status BEFORE score (col 5→status, col 6→score). In applications.md the order is inverted (col 5→score, col 6→status). merge-tracker.mjs handles the conversion.

**Valid canonical states:** `Evaluated`, `Applied`, `Replied`, `Interview`, `Offer`, `Rejected`, `Discarded`, `DO NOT APPLY`

Where `{next_num}` is calculated by reading the last line of `data/applications.md`.

### Step 6 — Final Output

Upon completion, print to stdout a JSON summary for the orchestrator to parse:
